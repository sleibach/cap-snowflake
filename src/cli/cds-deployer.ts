...
export async function deploy(_model: any, _db: string | undefined, options: any) {
  ...
  const rawCreds = (cds.env as any).requires?.db?.credentials as SnowflakeCredentials | undefined;
  if (!rawCreds) {
    throw new Error(
  ...
  const creds: SnowflakeCredentials = { ...rawCreds };
  if (!creds.host) creds.host = `${creds.account}.snowflakecomputing.com`;
  ...
  const client: SnowflakeSQLAPIClient | SnowflakeSDKClient = isSQLAPI
