...
export async function loadCsvData(
  ...
  client: SnowflakeSQLAPIClient | SnowflakeSDKClient
): Promise<CsvLoadResult> {
  const deployCds = (cds as any).deploy;
  ...
  const entries: Array<[string, any, string]> = await deployCds.prepare(model);

  let loaded = 0;
  let skipped = 0;

  for (const [filepath, entityNameOrDef, content] of entries) {
    if (!filepath.endsWith('.csv')) { skipped++; continue; }
    if (!content || typeof content !== 'string') { skipped++; continue; }
    if (!entityNameOrDef) { skipped++; continue; }

    // Entity name is either a string or a CSN entity definition
    const entityName: string = typeof entityNameOrDef === 'string'
      ? entityNameOrDef
    ...
    let rows: any[][];
    try {
      const parsed = (cds as any).parse.csv(content) as [string[], ...any[][]];
      [cols, ...rows] = parsed;
      if (!cols?.length || !rows.length) { skipped++; continue; }
    } catch (err: any) {
    ...
    const persistenceName: string =
      entityDef['@cds.persistence.name'] ??
    ...
    for (let start = 0; start < rows.length; start += BATCH_SIZE) {
      ...
      await (client as any).execute(sql, params);
    }
    loaded += rows.length;
  }
