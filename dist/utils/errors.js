...
export class SnowflakeError extends Error {
    ...
    status;
    constructor(message, code, sqlState, statusCode) {
        super(message);
...
export function normalizeError(error) {
    ...
    if (error.code && error.sqlState) {
        ...
        return new SnowflakeError(error.message || 'Unknown Snowflake error', error.code, error.sqlState, statusCode);
    }
    // HTTP error from SQL API
    if (error.response) {
        const data = error.response.data;
        return new SnowflakeError(data?.message || error.message || 'Snowflake SQL API error', data?.code || 'SNOWFLAKE_API_ERROR', data?.sqlState, error.response.status);
    }
    // Generic error
    return error;
}
 ...
 */
function mapSQLStateToHTTP(sqlState) {
    // SQL state classes
