 ...
 */
export function processExpands(baseQuery, expands, credentials, associationMap, params) {
 ...
 */
function addToOneJoins(baseQuery, expands, associationMap, credentials, params) {
    let sql = baseQuery;
        ...
        if (expand.expand && expand.expand.length > 0) {
            ...
            for (const col of expand.expand) {
                if (col.ref) {
                    ...
                    additionalColumns.push(`${joinAlias}.${quotedCol} AS ${quoteIdentifier(`${assocName}_${colName}`)}`);
                }
            }
        }
        else {
            // All columns: use OBJECT_CONSTRUCT for JSON object
            additionalColumns.push(`OBJECT_CONSTRUCT(*) AS ${quoteIdentifier(assocName)}_expand_data`);
        }
    }
 ...
 */
async function postProcessToMany(rows, _expands, _associationMap, _credentials) {
 ...
 */
export function buildJoinForAssociation(parentAlias, association, joinAlias, credentials) {
 ...
 */
export function restructureExpandedResults(rows, expands) {
    return rows.map(row => {
        const result = { ...row };
        for (const expand of expands) {
            const assocName = expand.ref[expand.ref.length - 1];
 ...
 */
export function generateToManyExpand(parentTable, parentAlias, association, credentials, expandColumns) {
