...
async function executeStatements(credentials, statements, opts) {
    if (opts.dry) {
    ...
    for (const stmt of statements) {
        ...
        try {
            ...
            const sqlState = err?.response?.data?.sqlState ?? err?.sqlState ?? '';
...
async function main() {
    ...
    const credentials = resolveEnvRefs(rawCreds);
    if (!credentials.host) {
    ...
    const model = await cds.load('*', { root: projectRoot }).catch(async () => {
    ...
    if (!dry) {
        ...
        if (migrate && failed === 0) {
            ...
            const rows = await client.execute(`SELECT TABLE_NAME, COLUMN_NAME
         ...
         WHERE TABLE_SCHEMA = '${schema.toUpperCase()}'`);
            ...
            for (const row of rows) {
                ...
                if (!existingCols.has(tbl))
                    existingCols.set(tbl, new Set());
